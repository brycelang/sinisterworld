(function(_, $) {
    ...
    function initGoogleFonts()
    {
        ...
        container.each(function() {
            var self = $(this);
            var elms;
            var text = $.rtrim($.ltrim(self.data('caSelectBoxDefault'), "'\""), "'\"");
            ...
            var active = self.find('ul li').filter('li[data-ca-select-box-value="' + text + '"]');
            if (active.length) {
                active.addClass('active');
                self.find('span:first').html(text.replace(/(,sans-serif|,serif|,cursive)/, ""));
            }
        });
    }
    ...
    $.ceEvent('on', 'ce.switch_theme_editor_container', function(flag) {
        $('#sw_theme_editor_container i').toggleClass('hidden');
        if (flag) {
